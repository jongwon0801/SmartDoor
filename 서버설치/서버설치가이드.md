...
```less
# PW는 홈페이지 비밀번호와 모듈 비밀번호가 따로 존재함
# 모듈 비밀번호를 입력해야함 wiki0800**
# 홈페이지 초기 비밀번호가 모듈 비밀번호 -> 모르면 변경하면 됨
# uds.conf에는 항상 모듈비밀번호를 입력해야함
# master or slave 계정 둘다 따로 모듈 비밀번호 설정 가능
# 각 계정마다 설정할 ip를 매칭해야 동작함 (회사에 승인필요)

# 자바가 클라이언트로 먼저 TCP 연결을 생성
공유기 인바운드 포트 개방은 필요 없음.
TCP는 한 번 연결되면 양방향 송수신 가능하므로
서버(비즈뿌리오)가 해당 연결로 데이터를 푸시할 수 있음.
```

#### ss -tnp  /  netstat -tnp
```less
ESTAB     [::ffff:192.168.0.73]:47712       [::ffff:27.102.215.25]:18300     users:(("java",pid=45434,fd=38))    
ESTAB     [::ffff:192.168.0.73]:46794       [::ffff:27.102.215.25]:18400     users:(("java",pid=45434,fd=39)) 
```
...
$_lib['url']['api'] = '//api2.hizib.wikibox.kr';			//api 사이트
$_lib['url']['app'] = 'https://api2.hizib.wikibox.kr/download';	//APP 사이트
...
#### 10-4. 서버 주소 사용하는 파일 수정 (로컬 디바이스, 서버 둘다)
```less
...
#### 10-5. 서버 주소 사용하는 파일 수정 (로컬디바이스, 서버 둘다)
```less
grep -rl "13.124.155.19" ./

grep -rl "13.124.155.19" ./ | xargs sed -i 's/13\.124\.155\.19/175.211.153.28/g'
