...
#### 보관함 네이버 서버 DB 접속 125.209.200.159
```less
sudo mysql -u root -p
...
#### MariaDB 서버에서 현재 사용자 인증 플러그인 확인
```less
mysql> SELECT user, host, plugin FROM mysql.user WHERE user = 'yellowbox';
+-----------+-----------+-----------------------+
| user      | host      | plugin                |
+-----------+-----------+-----------------------+
| yellowbox | %         | mysql_native_password |
| yellowbox | localhost | mysql_native_password |
+-----------+-----------+-----------------------+
```
