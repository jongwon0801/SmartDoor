...
# HDMI sink를 기본 출력으로 활성화 2번 hdmi 내측스피커
pactl set-default-sink alsa_output.platform-fef00700.hdmi.hdmi-stereo

# usb 외측 스피커 0번 스피커로 설정
pactl set-default-sink alsa_output.usb-C-Media_Electronics_Inc._USB_Audio_Device-00.analog-stereo

# --aout=pulse 옵션은 VLC가 PulseAudio 출력으로 강제 사용하도록 지정
cvlc --aout=pulse --play-and-exit /home/pi/guest_voice/voice_record_no.wav

# HDMI 오디오 강제로 깨우기 켜기 (활성화)
pactl suspend-sink alsa_output.platform-fef00700.hdmi.hdmi-stereo 0

# HDMI 오디오 강제로 끄기 (비활성화)
pactl suspend-sink alsa_output.platform-fef00700.hdmi.hdmi-stereo 1

# PulseAudio에 출력 강제로 변경
pactl set-default-sink alsa_output.platform-fef00700.hdmi.hdmi-stereo
