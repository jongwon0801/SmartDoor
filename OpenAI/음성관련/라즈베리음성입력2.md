...
* 12 default, ALSA (32 in, 32 out)
   ...
   0 bcm2835 Headphones: - (hw:0,0), ALSA (0 in, 8 out)
   1 vc4-hdmi-0: MAI PCM i2s-hifi-0 (hw:1,0), ALSA (0 in, 6 out)
   2 USB Audio Device: - (hw:2,0), ALSA (0 in, 2 out)
   3 USB 4K Live Camera: Audio (hw:4,0), ALSA (1 in, 0 out)
   4 sysdefault, ALSA (0 in, 128 out)
   5 lavrate, ALSA (0 in, 128 out)
   6 samplerate, ALSA (0 in, 128 out)
   7 speexrate, ALSA (0 in, 128 out)
...
설명:
인덱스 3번에 해당하는 **입력 장치(input)**의 세부 정보를 출력합니다.

용도:
특정 마이크의 지원 샘플레이트, 입력 채널 수, 디바이스 이름 등을 확인할 수 있습니다.

3는 위에서 확인한 장치 번호입니다 (입력 전용일 수도, 입력+출력 겸용일 수도 있음).
...
# 즉, 2번 장치는 입력 채널이 없기 때문에 마이크로 쓸 수 없다는 뜻입니다.
ValueError: Not an input device: 'vc4-hdmi-0: MAI PCM i2s-hifi-0 (hw:2,0)'
