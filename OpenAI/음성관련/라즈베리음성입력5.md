...
```
#### aout
"audio output" 즉, 오디오 출력 방식(audio output module) 을 지정

```less
--aout 옵션은 VLC가 소리를 어떻게 내보낼지 정하는 설정이에요.

예를 들어 ALSA, PulseAudio, OSS, DirectSound 같은 여러 오디오 시스템 중에서 선택할 수 있어요.
```
```less
--aout=alsa : ALSA (Advanced Linux Sound Architecture)를 통해 소리를 출력

--aout=pulse : PulseAudio를 통해 소리를 출력

--aout=oss : OSS (Open Sound System) 통해 출력 (리눅스 예전 방식)

--aout=directsound : 윈도우즈의 DirectSound 사용
```
