# nano /home/pi/www/python/elcsoft/controller/smartdoor.py
...
def isOwner() :
    #로컬DB에서 확인 하는 것으로 변경
    url = 'https://api.hizib.wikibox.kr/Smartdoor/owner'
...
def getToken(**kwargs) :
    ...
    if isReresh :
        ...
        url = 'https://api.hizib.wikibox.kr/Smartdoor/token?smartdoor_id=%s' % kwargs['smartdoor_id']
...
def setBle(address) :
    ...
    if smartdoorObj.ble != address :
        ...
        url = "https://api.hizib.wikibox.kr/Smartdoor"
        data = {"ble": address}
        token = getToken()
...
def joinProcess(code) :
    ...
    url = 'https://api.hizib.wikibox.kr/Smartdoor/findByCode?code=%s' % code
...
def downloadProcess(**kwargs):
    ...
    url = 'https://api.hizib.wikibox.kr/Smartdoor/%s' % pkValue
