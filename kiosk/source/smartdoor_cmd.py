# /home/pi/www/python/elcsoft/controller/smartdoor_cmd.py 
...
def guestkeyJoinProcess(**kwargs):
    ...
    result = lib.restapi(method="POST", url="https://api.hizib.wikibox.kr/Smartdoor/guestkeyJoinProcess", data={"user_id": int(user_id), "handphone": handphone, "passwd": passwd, "startDate": startDate, "stopDate": stopDate}, token=token)
...
def setIsDoorOpenProcess(isOpen):
    ...
    url = 'https://api.hizib.wikibox.kr/Smartdoor/setIsDoorOpen'
    data = dict()
    if isOpen:
        data['isDoorOpen'] = 1
    else:
