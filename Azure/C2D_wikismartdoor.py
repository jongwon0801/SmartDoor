...
class WikiSmartdoor:
    ...
    def message_handler(message):
        try:
            ...
            logger.Logger._LOGGER.warning(f"메시지 디코딩 오류: {e}")
            msg = ""

        logger.Logger._LOGGER.info(f"Received message: {msg}")
        logger.Logger._LOGGER.info(f"Properties: {message.custom_properties}")
    ...
    logger.Logger._LOGGER.info("Listening for messages...")
    client.connect()
    ...
    try:
        ...
        logger.Logger._LOGGER.info("Stopping...")
    finally:
        client.shutdown()
