...
# 시스템 의존성 패키지 설치
# build-essential, cmake, gcc, gfortran, pkg-config는 컴파일에 필요합니다.
# python3-dev는 파이썬 개발 헤더를 제공하며,
# libsm6, libxext6, libxrender-dev, libgtk-3-dev는 GUI/이미지 처리 라이브러리 (예: OpenCV)에 필요합니다.
# libblas-dev, liblapack-dev, python3-numpy, python3-scipy는 과학 계산 라이브러리(numpy, scipy)의 시스템 의존성을 해결합니다.
RUN apt-get update && apt-get install -y \
    build-essential \
    cmake \
    libsm6 \
...
# setuptools와 wheel을 먼저 설치하여 빌드 환경을 준비합니다.
RUN pip install --upgrade pip
RUN pip install --no-cache-dir setuptools wheel

# requirements.txt에 있는 다른 패키지들이 numpy에 의존할 경우를 대비하여,
# --only-binary :all: 옵션을 사용하여 미리 컴파일된 바이너리를 우선적으로 사용하도록 합니다.
# 또한, ARM 아키텍처에서 빌드 성공률이 높은 특정 numpy 버전을 지정하는 것도 좋은 방법입니다.
# 만약 requirements.txt에 numpy가 아예 없다면, 이 부분은 특정 버전에 얽매이지 않고 진행해도 무방합니다.
