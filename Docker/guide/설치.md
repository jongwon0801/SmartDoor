...
# NumPy를 먼저 설치 (ARM 호환 버전)
RUN pip install --no-cache-dir numpy==1.19.5

# requirements.txt에서 numpy를 제외한 나머지 패키지 설치
RUN grep -v "numpy" /app/requirements.txt > /app/requirements_without_numpy.txt && \
    pip install --no-cache-dir -r /app/requirements_without_numpy.txt
